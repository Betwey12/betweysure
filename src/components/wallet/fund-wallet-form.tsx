...
import fonbnk from "@/assets/icons/fonbnk.png";
...
export default function FundWalletForm() {
  ...
  const currency =
    watchPaymentMethod === "fonbnk" ? "USD" : (user?.currency ?? "NGN");
  ...
  const onSubmit: SubmitHandler<PaymentForm> = async (data) => {
    if (!user?._id) return toast("Something went wrong!");

    if (data.paymentMethod === "fonbnk") {
      const params = new URLSearchParams({
        ...fonbnkDetails,
        orderParams: user?._id,
        amount: data.amount.toString(),
        freezeAmount: data.amount.toString(),
      }).toString();

      return router.push(`https://pay.fonbnk.com/?${params}`);
    }
          ...
          {watchPaymentMethod === "bank-transfer" ? (
            ...
            <>
              ...
              <Button className="self-start mt-6 gap-4">
                Fund Wallet {formatCurrency(watchAmount, currency)}
                {isPending && (
